...
var TILEDMapClass = Class.extend({
    ...
    parseMapJSON: function (mapJSON) {
        ...
        gMap.checkWait(
            function()
            {
                    // ...Once all the tilesets are loaded, 
                    // set the 'fullyLoaded' flag to true...
                        return gMap.fullyLoaded;
            },
            function () 
            {
                        gMap.createCollision();
    ...
    checkWait: function(conditionFunction, resultFunction)
                {
    ...
    createCollision: function(){
        ...
        var entityObjects = 0;
        var sensorsObjects = 0;
        var that = this;

        for(var i = 0; i < map.layers.length; i++) {
            if( map.layers[i].name == "collision") {
                collisionObjects = map.layers[i].objects;
            }
            if(map.layers[i].name == "entities"){
                entityObjects = map.layers[i].objects;
            }

            if(map.layers[i].name == "sensors"){
                sensorsObjects = map.layers[i].objects;
            }
        }
        ...
        if(collisionObjects) {
            collisionObjects.forEach(function(object){
                ...
                var entityDef = {
                    id: "MapCollision",
                    type: "static",
                    x: object.x + (map.tilewidth/2) * (object.width/map.tilewidth),
                    y: object.y + (map.tileheight/2) * (object.height/map.tileheight),
                    Width: object.width,
                    Height: object.height,
                    polygonPoints: object.polygon,
                    userData: {
                        id: "MapCollision"
                    }
                };
                ...
                if(object.polygon){
                    gPhysicsEngine.addPolygonBody(entityDef);
                }else {
        ...
        if(entityObjects){
            entityObjects.forEach(function(entity){
                if(entity.properties.crop) {
                    entity.properties.crop = {
                        w : entity.properties.crop.split(",")[0],
                        h : entity.properties.crop.split(",")[1]
                    };
                }

                if(entity.properties.drawOffset) {
                    entity.properties.drawOffset = {
                        x : entity.properties.drawOffset.split(",")[0],
                        y : entity.properties.drawOffset.split(",")[1]
                    };
                }

                entity.properties.width = entity.width;
                entity.properties.height = entity.height;
                entity.properties.rotation = entity.rotation;



                gGameEngine.spawnEntity(entity.type, entity.x, entity.y, entity.properties, entity.name);
            });

        }

        if(sensorsObjects) {
            sensorsObjects.forEach(function(sensor) {

                sensor.properties.width = sensor.width;
                sensor.properties.height = sensor.height;
                sensor.properties.rotation = sensor.rotation;

                gGameEngine.spawnEntity(sensor.type, sensor.x, sensor.y, sensor.properties, sensor.name);
    ...
    centerAt: function(x, y) {
        ...
        gMap.viewRect.x = x - canvas.width/2;
        gMap.viewRect.y = y - canvas.height/2;
        
        if(gMap.viewRect.y < 0)gMap.viewRect.y = 0;
        if(gMap.viewRect.y > gMap.pixelSize.y - canvas.height)gMap.viewRect.y = gMap.pixelSize.y - canvas.height;
        if(gMap.viewRect.x > gMap.pixelSize.x - canvas.width)gMap.viewRect.x = gMap.pixelSize.x - canvas.width;
        if(gMap.viewRect.x < 0) gMap.viewRect.x = 0;
        gMap.viewRect.w = canvas.width;
        gMap.viewRect.h = canvas.height;
    },
    ...
    draw: function (ctx) {
        ...
        for(var q = 0; q < gMap.canvasTileArray.length; q++) {
            ...
            if(r1.isVisible()) ctx.drawImage(r1.cvsHdl, r1.x, r1.y);
