...
GameEngineClass = Class.extend({
	...
	setup: function () {
		...
		gPhysicsEngine.addContactListener({
			...
			postSolve: function (body A, bodyB, impulse) {


				var uA = bodyA.GetUserData();
				var uB = bodyB.GetUserData();

				if(uA !== null && uA.ent !== null && uA.ent.onTouch) {
					uA.ent.onTouch(bodyB, null, impulse);
				}
				
				if(uB !== null && uB.ent !== null && uB.ent.onTouch) {
    ...
    //-------------------------------------------------
    draw: function(){
    	gMap.draw(null) //TODO canvas context
    	...
    	this.entities.forEach(function(entity) {
    		...
    		if(entity.pos.x >= gMap.viewRect.x - fudgeVariance &&
    			entity.pos.y < gMap.viewRect.x + gMap.viewRect.w + fudgeVariance &&
    	...
    	zIndex.array.forEach(function(zIndex){
